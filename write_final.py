      ...
      <div className=\"relative z-10\">
        ...
        <div className=\"marquee\">
          {allSkills.map((skill, index) => (
            <div key={\marq-1-\\} className=\"flex items-center justify-center whitespace-nowrap\">
          ...
          {allSkills.map((skill, index) => (
            <div key={\marq-2-\\} className=\"flex items-center justify-center whitespace-nowrap\">
              <div className=\"px-6 py-3 bg-gradient-to-r from-cyan-500/20 to-blue-500/20 border border-cyan-500/50 rounded-full hover:border-cyan-500 transition-colors duration-300 cursor-pointer group\">
                <span className=\"text-white font-medium text-sm group-hover:text-cyan-300 transition-colors\">
                  {skill}
                </span>
        ...
        <div className=\"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8\">
          {skillCategories.map((category, idx) => (
            <div key={idx} className=\"p-6 bg-gradient-to-br from-slate-900/50 to-slate-800/50 border border-slate-700/50 rounded-lg hover:border-cyan-500/50 transition-colors duration-300\">
              ...
              <div className=\"flex flex-wrap gap-2\">
                {category.skills.map((skill, idx) => (
                  <span key={idx} className=\"px-3 py-1 text-sm bg-slate-700/50 text-slate-200 rounded-md hover:bg-cyan-500/20 hover:text-cyan-300 transition-colors\">
                    {skill}
                  </span>
                ))}
              </div>
