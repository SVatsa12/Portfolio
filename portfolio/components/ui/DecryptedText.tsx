...
type DecryptedTextProps = Omit<ComponentProps<typeof motion.span>, "children"> & {
  text: string;
  speed?: number;
  ...
  useEffect(() => {
    ...
    const observerCallback: IntersectionObserverCallback = entries => {
      entries.forEach(entry => {
        ...
        const isReEntering = !wasInViewRef.current;
        wasInViewRef.current = true;

        if (isReEntering && (repeatOnView || !hasAnimated) && !isAnimating) {
          triggerDecrypt();
          setHasAnimated(true);
        }
      });
